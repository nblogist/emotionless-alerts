...
export async function analyzeNewsWithAI(articles, portfolioCoins) {
  ...
  const prompt = `Analyze these crypto/finance news headlines. Portfolio holds: ${coins}.

${headlines}

Return a JSON array of ONLY headlines that could materially move prices or signal risk. For each:
- "num": headline number (1-indexed)
- "coins": affected coins from the portfolio, or ["ALL"] for macro events
- "direction": "bullish" | "bearish" | "neutral"
- "insight": 1-sentence actionable take (under 80 chars)

Include macro events (central bank decisions, economic data, geopolitical, market structure shifts) that affect ALL crypto.

Be selective — only genuinely impactful news. Return [] if nothing is material. JSON array only, no markdown.`;
