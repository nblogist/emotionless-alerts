...
const TEMPLATES = {
  BUY_DIP: (s) =>
    `${s.asset} is ${pct(Math.abs(s.deviation))} below its target slice — buying ${usd(s.buyAmountUsd)} to close the gap and lower your average cost.${s.capped ? ` (Capped from the ideal ${usd(s.idealGapUsd)} — ${s.cappedReason}.)` : ''}`,
  SKIM: (s) =>
