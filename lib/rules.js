...
function fmtCoin(n) {
  return Number(n).toFixed(n >= 1 ? 4 : 6);
}

// How much $ to deploy per rung: powder / 5 (5 rungs of buying)
  ...
  if (price <= threshold) {
    if (!(await wasAlerted(key))) {
      ...
      return [
        `🟢 BUY ${coin} NOW`,
        ``,
        `Price: ${fmtUsd(price)} (down ${(config.buyBandPct * 100).toFixed(0)}% from your last buy at ${fmtUsd(ref)})`,
        ``,
        `What to do:`,
...
export async function checkFloorConfirmed(coin, weeklyCloses, config) {
...
export async function checkThesisBreak(btcWeeklyCloses, ma200) {
  ...
  if (last2[0] < ma200 && last2[1] < ma200) {
    if (!(await wasAlerted(key))) {
      await markAlerted(key);
      return [
        `🚨 THESIS BREAK: BTC below 200-week MA`,
        ``,
...
export async function checkUpsideBreak(btcWeeklyCloses, upsideBreakUsd, config) {
  ...
  if (lastClose > upsideBreakUsd) {
    if (!(await wasAlerted(key))) {
      ...
      return [
        `🚀 UPSIDE BREAKOUT: BTC above ${fmtUsd(upsideBreakUsd)}`,
        ``,
...
export async function checkMonthly(config, prices) {
  ...
  return [
    `📊 MONTHLY REVIEW — Take 10 minutes to check your positions`,
    ``,
    ...lines,
    ``,
