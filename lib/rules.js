...
export async function checkBuyDip(asset, price, portfolio, pid, pName, recentHigh) {
  ...
  // Path (a): price below avg cost
  const belowCost = price < asset.avgCost;
  ...
  const discount = ((asset.avgCost - price) / asset.avgCost * 100).toFixed(1);
  const dropPct = (highDrop * 100).toFixed(1);

  const headline = buyReason === 'below_cost'
    ? `${tag(pName)} BUY ${asset.symbol} — ${discount}% below your avg cost`
    ...
    return {
      ...
      message: [
        ...
        buyReason === 'below_cost'
          ? `Price: ${fmtUsd(price)} vs your avg cost ${fmtUsd(asset.avgCost)} (${discount}% discount).`
  ...
  return {
    ...
    message: [
      ...
      ``,
      buyReason === 'below_cost'
        ? `Price: ${fmtUsd(price)} (${discount}% below your avg cost of ${fmtUsd(asset.avgCost)})`
        : `Price: ${fmtUsd(price)} — down ${dropPct}% from its ${STRATEGY_CONFIG.liquidBasket.recentHighWindowDays}-day high of ${fmtUsd(recentHigh)}`,
