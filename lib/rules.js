...
// ══════════════════════════════════════════════════════════════
// 1. BUY THE DIP — any asset ≥10% below target (continuous)
// Size = $ to bring asset back to target, capped at spendable cash
// ══════════════════════════════════════════════════════════════
export async function checkBuyDip(asset, price, portfolio, pid, pName) {
  ...
  // Only fire when ≥10% below target
  if (dev > -0.10) return null;

  const key = `${pid}:buyDip:${asset.symbol}`;

  // Transition-only: don't re-alert while still below target
  if (await wasAlerted(key)) return null;

  const idealGap = targetVal - currentVal;
  const spendable = getSpendableCash(portfolio);
  const buyAmount = Math.min(idealGap, spendable);

  if (buyAmount <= 0) {
    // Can't buy — capped out. Still mark so we don't re-alert.
    ...
    return {
      ...
      message: [
        `${tag(pName)} BUY ${asset.symbol} — ${fmtPct(Math.abs(dev))} below target`,
        ``,
        `${asset.symbol} is at ${fmtUsd(currentVal)} vs target ${fmtUsd(targetVal)} (${fmtPct(Math.abs(dev))} below).`,
        `Ideal buy: ${fmtUsd(idealGap)} to bring it back to target.`,
  ...
  const capped = buyAmount < idealGap;
  const floor = portfolio.portfolioValue * 0.10;
  ...
  return {
    ...
    message: [
      ...
      ``,
      `Price: ${fmtUsd(price)}`,
      `Current value: ${fmtUsd(currentVal)} vs target ${fmtUsd(targetVal)} (${fmtPct(Math.abs(dev))} below).`,
      ``,
      `What to do:`,
      `  1. Buy ${fmtCoin(coinsToBuy)} ${asset.symbol} at ${fmtUsd(price)}`,
      `  2. Spend: ${fmtUsd(buyAmount)}${capped ? ` (capped — ideal gap is ${fmtUsd(idealGap)}, but only ${fmtUsd(spendable)} spendable above 10% floor)` : ` (brings ${asset.symbol} back to target)`}`,
      `  3. Cash after: ${fmtUsd(portfolio.cash - buyAmount)}`,
    ].join('\n'),
  };
}

// Clear buy-dip alert when asset recovers above trigger threshold
export async function clearBuyDipIfRecovered(asset, price, portfolio, pid) {
  const targetVal = getTargetValue(asset, portfolio);
  const currentVal = asset.currentValue || 0;
  const dev = getDeviation(currentVal, targetVal);
  if (dev > -0.10) {
